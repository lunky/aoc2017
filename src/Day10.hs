...
day10b input window = concatMap hex $ map(\x -> (foldl (\acc y -> y ^ acc) 0) x) $ 
                chunks 16 $ 
                runTwists window 64 $ 
                (map ord input) ++ [17, 31, 73, 47, 23]
                where (^) = xor
                
runTwists window iterations counts = (\(result,_,_)->result) $foldl' (\acc y -> twistb acc counts) (window,0,0) [1..iterations] 

day10b'  window iterations counts  = concatMap hex $ map(\x -> (foldl (\acc y -> y `xor` acc) 0) x) $ chunks 16 $ runTwists window iterations counts
