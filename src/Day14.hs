...
day14b :: String -> Int
day14b input = length $ twoDimensionGroup $ day14' input
...
matchingAdjacent x y = ((snd x) == (snd y))  && (elem (fst x) $ adjacentCoordinates (fst y))
...
match x xs = nub $ (foldr (\y acc -> match' y xs) initial initial)
    where initial = match' x xs

match' x xs = filter (\y -> (matchingAdjacent x y) || y==x ) xs
    
qgroup []           =  []
qgroup (x:xs)       =  (x:ys) : qgroup zs
                           where (ys,zs) = matching x xs -- span (== x) xs

deconstruct list = map (\y -> map(\(_,x) -> x) y) list 
out = [((0,1),'b'),((1,1),'b'),((3,1),'b')]
out2 = [((0,0),'a'),((1,0),'a'),((3,0),'a')]
out3 = [((0,0),'a'),((1,0),'a'),((2,0),'a')]